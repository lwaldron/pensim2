...
This package was developed in support of the study by @waldron_optimized_2011.  This paper contains greater detail
...
```{r ROCplot, fig.cap="ROC plot of cross-validated continuous risk predictions at 12 months. Note that the predictions are better if you don't randomly select 250 genes to start with!  We only did this to ease the load on the CRAN checking servers."}
library(survivalROC)
nobs <- length(preds)
cutoff <- 12
preds.roc <-
...
```{r heatmap, fig.cap = "Heatmap of simulated data with binary response."}
...
```{r simulatedKM, fig.cap = "Kaplan-Meier plot of survival of simulated cohort."}
library(survival)
